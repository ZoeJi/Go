...
public class PostDumpFlow {
    private static final String PROJECT_ID = "around-zoe";
    ...
    public static void main(String[] args) {
        ...
        PCollection<Result> btRows = p.apply(Read.from(CloudBigtableIO.read(config))); // read rows
        PCollection<TableRow> bqRows = btRows.apply(ParDo.of(new DoFn<Result, TableRow>() { // bigtable -> bigquery
            @Override
            public void processElement(ProcessContext c) {
                Result result = c.element();
                // read from result (BigTable rows)
                ...
                TableRow row = new TableRow();//create BigQuery Table row object
        ...
        // write bqRows to BigQuery schema
